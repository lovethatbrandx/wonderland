...
export async function fetchModels(baseUrl) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      const res = await fetchWithTimeout(`${url}/models`, {}, 10000);
      ...
      lastError = err;
      console.error(`[LMSTUDIO] fetchModels attempt ${attempt} failed:`, err.message);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!res.ok) {
        ...
        console.error('[LMSTUDIO] Error response:', err);
        throw new Error(err || `LM Studio error: ${res.status}`);
      }

      return parseStreamingResponse(res, onChunk);
    } catch (err) {
      lastError = err;
      console.error(`[LMSTUDIO] chatCompletion attempt ${attempt} failed:`, err.message);
